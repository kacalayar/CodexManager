...
import { onRequestLog, type RequestLog } from "../lib/tauri/logs";
...
type QuickFilter = "" | "completions" | "embeddings" | "images" | "__ERROR__" | "gemini" | "claude";
...
export function LogViewerPage() {
  ...
  const stats = createMemo(() => {
    const all = requestStore.history().requests;
  ...
  const filteredRequestLogs = createMemo(() => {
    let result = requestStore.history().requests;
    ...
    if (qf === "__ERROR__") {
      ...
      result = result.filter(
        (r) =>
          r.provider.toLowerCase().includes("gemini") ||
          r.model.toLowerCase().includes("gemini") ||
          r.path.toLowerCase().includes("gemini"),
      );
    } else if (qf === "claude") {
      result = result.filter(
        (r) =>
          r.provider.toLowerCase().includes("anthropic") ||
          r.model.toLowerCase().includes("claude") ||
          r.path.toLowerCase().includes("anthropic") ||
          r.path.toLowerCase().includes("messages"),
      );
    } else if (qf) {
