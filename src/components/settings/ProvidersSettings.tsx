...
import { detectCopilotApi, getAvailableModels, isCopilotModel } from "../../lib/tauri";
...
export function ProvidersSettings(props: ProvidersSettingsProps) {
  ...
  const oauthModelsBySource = createMemo(() => {
    // Filter models that come from OAuth sources
    const oauthModels = models().filter((m) => {
      const source = m.source?.toLowerCase() || "";
      // Include models from OAuth, Copilot, or any source containing "oauth"
      return source.includes("oauth") || source.includes("copilot") || source === "oauth";
    });

    // Group by ownedBy (provider) instead of source
    ...
    for (const model of oauthModels) {
      ...
      if (isCopilotModel(model)) {
        ...
        providerKey = "OpenAI";
      } else if (model.ownedBy === "google" || model.ownedBy === "antigravity") {
        providerKey = "Gemini";
      } else if (model.ownedBy === "anthropic") {
        providerKey = "Claude";
      } else {
      ...
      grouped[providerKey].push(model.id);
    }
    return grouped;
  });
  ...
  const connectedCount = () => {
    ...
    return [auth.openai, auth.gemini, auth.antigravity].filter(Boolean).length;
  };
