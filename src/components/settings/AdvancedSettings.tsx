...
export function AdvancedSettings(props: AdvancedSettingsProps) {
  ...
  const getAvailableTargetModels = () => {
    ...
    const groupedModels = {
      anthropic: models
        .filter((m) => m.ownedBy === "anthropic" && !isCopilotModel(m))
        .map((m) => ({ label: m.id, value: m.id })),
      copilot: models.filter((m) => isCopilotModel(m)).map((m) => ({ label: m.id, value: m.id })),
      google: models
        .filter(
          (m) => (m.ownedBy === "google" || m.ownedBy === "antigravity") && !isCopilotModel(m),
        )
        .map((m) => ({ label: m.id, value: m.id })),
      ...
      kiro: models
        ...
        .map((m) => ({ label: m.id, value: m.id })),
      openai: models
        .filter((m) => m.ownedBy === "openai" && !isCopilotModel(m))
        .map((m) => ({ label: m.id, value: m.id })),
    ...
    <div class="space-y-4">
                  ...
                  fallback={
                    <div class="text-center">
                      ...
                      <a
                        ...
                        href="https://github.com/kacalayar/CodexManager/releases"
      ...
      <div class="space-y-4">
        ...
        <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 text-center dark:border-gray-700 dark:bg-gray-800/50">
          ...
          <p class="mt-2 text-xs text-gray-400 dark:text-gray-500">Built with love by SR</p>
        </div>
      </div>
    </div>
  );
