...
function createAppStore() {
  ...
  const [proxyStatus, setProxyStatus] = createSignal<ProxyStatus>({
    endpoint: "http://localhost:8320/v1",
    port: 8320,
    running: false,
  });
  ...
  const updateProxyStatus = (status: ProxyStatus, showNotification = false) => {
    ...
    if (status.running && !wasRunning) {
      ...
      if (showNotification) {
        showSystemNotification("CodexManager", "Proxy server is now running");
      }
    } else if (!status.running && wasRunning) {
      setProxyStartTime(null);
      if (showNotification) {
        showSystemNotification("CodexManager", "Proxy server has stopped");
      }
    }
  };
