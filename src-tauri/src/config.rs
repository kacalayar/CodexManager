...
use crate::types::{
    ...
    ClaudeApiKey, CodexApiKey, CopilotConfig, GeminiApiKey, SshConfig,
};
...
fn default_management_key() -> String {
    "CodexManager-mgmt-key".to_string()
}

fn default_proxy_api_key() -> String {
    "CodexManager-local".to_string()
}
...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            ...
            proxy_api_key: "CodexManager-local".to_string(),
            management_key: "CodexManager-mgmt-key".to_string(),
...
/// Get the CodexManager config directory, creating it if needed
pub fn get_codex_manager_config_dir() -> std::path::PathBuf {
    let config_dir = dirs::config_dir()
        .unwrap_or_else(|| {
            eprintln!(
                "[CodexManager] Warning: Could not determine config directory, using current directory"
            );
    ...
    if let Err(e) = std::fs::create_dir_all(&config_dir) {
        eprintln!(
            "[CodexManager] Error: Failed to create config directory '{}': {}",
            config_dir.display(),
            e
        );
    }
...
fn migrate_config(config: &mut AppConfig) -> bool {
    if let Some(old_provider) = config.amp_openai_provider.take() {
        if config.amp_openai_providers.is_empty() {
            eprintln!("[CodexManager] Migrating config from old provider format to array format...");
            eprintln!(
                "[CodexManager] Old provider: {} with {} models",
                old_provider.name,
                old_provider.models.len()
            );
            for (i, model) in old_provider.models.iter().enumerate() {
                eprintln!("[CodexManager]   Preserving model {}: {}", i, model.name);
            }
            ...
            eprintln!("[CodexManager] Config migration complete");
            return true;
        }
    }
...
fn load_config_from_path(path: &Path) -> AppConfig {
    ...
    let data = match std::fs::read_to_string(path) {
        ...
        Err(e) => {
            eprintln!(
                "[CodexManager] Failed to read config file '{}': {}. Falling back to defaults.",
                path.display(),
                e
            );
            return AppConfig::default();
    ...
    let mut config = match serde_json::from_str::<AppConfig>(&data) {
        ...
        Err(e) => {
            eprintln!(
                "[CodexManager] Failed to parse config file '{}': {}. Falling back to defaults.",
                path.display(),
                e
            );
            return AppConfig::default();
...
pub(crate) fn save_config_to_path(path: &Path, config: &AppConfig) -> Result<(), String> {
    ...
    for attempt in 0..3 {
        match std::fs::write(&temp_path, &data) {
            ...
            Err(e) => {
                ...
                if attempt < 2 {
                    eprintln!(
                        "[CodexManager] Save attempt {} failed, retrying: {}",
                        attempt + 1,
                        e
                    );
    ...
    eprintln!("[CodexManager] Config saved successfully to: {:?}", path);
...
mod tests {
    ...
    fn test_dir(prefix: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("CodexManager-{}-{}", prefix, generate_uuid()));
