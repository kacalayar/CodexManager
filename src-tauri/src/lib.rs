...
use crate::types::{ProxyStatus, AuthStatus, CopilotStatusMap};
...
fn handle_deep_link(app: &tauri::AppHandle, urls: Vec<url::Url>) {
    for url in urls {
        if url.scheme() == "CodexManager" && url.path() == "/oauth/callback" {
...
fn setup_tray(app: &tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let quit_item = MenuItem::with_id(app, "quit", "Quit CodexManager", true, None::<&str>)?;
    ...
    let _tray = TrayIconBuilder::new()
        ...
        .tooltip("CodexManager - Proxy stopped")
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                let config = crate::config::load_config();
                // Auto-start all enabled copilot instances
                for instance in config.copilot.instances.iter() {
                    if !instance.enabled {
                        continue;
                    }
                    println!("[Copilot] Auto-starting copilot-api for instance: {} (port {})", instance.name, instance.port);
                    // Small delay to let the app fully initialize
                    tokio::time::sleep(tokio::time::Duration::from_secs(1)).await;
                    match commands::copilot::start_copilot_instance_internal(app_handle.clone(), instance.id.clone()).await {
                        Ok(status) => println!("[Copilot] Auto-start successful for {}: running={}", instance.name, status.running),
                        Err(e) => eprintln!("[Copilot] Auto-start failed for {}: {}", instance.name, e),
                    }
                }
            });
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // Multi-instance Copilot
            commands::copilot::get_all_copilot_statuses,
            commands::copilot::start_copilot_instance,
            commands::copilot::stop_copilot_instance,
            commands::copilot::stop_all_copilot_instances,
            // Auth & OAuth
                        ...
                        if let tauri::WindowEvent::CloseRequested { api, .. } = win_event {
                            ...
                            if close_to_tray {
                                ...
                                if let Some(window) = app_handle.get_webview_window("main") {
                                    println!("[CodexManager] Hiding to system tray...");
                                    let _ = window.hide();
                                }
                                api.prevent_close();
                            }
                    ...
                    if let Some(state) = app_handle.try_state::<AppState>() {
                        ...
                        if let Ok(mut process_guard) = state.proxy_process.lock() {
                            if let Some(child) = process_guard.take() {
                                println!("[CodexManager] Shutting down cliproxyapi...");
                                let _ = child.kill();
                            }
                        }
                        // Kill copilot-api process
                        if let Ok(mut processes) = state.copilot_processes.lock() {
                            let instance_ids: Vec<String> = processes.keys().cloned().collect();
                            for instance_id in instance_ids {
                                if let Some(child) = processes.remove(&instance_id) {
                                    println!("[CodexManager] Shutting down copilot-api instance: {}", instance_id);
                                    let _ = child.kill();
                                }
                            }
                        }
