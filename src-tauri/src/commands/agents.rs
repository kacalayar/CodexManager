...
pub fn detect_cli_agents(state: State<AppState>) -> Vec<AgentStatus> {
    ...
    let droid_configured = if droid_config.exists() {
        std::fs::read_to_string(&droid_config)
            .map(|c| c.contains(&endpoint) || c.contains("127.0.0.1:8320"))
            .unwrap_or(false)
    } else {
        false
    };
    ...
    let amp_configured = check_env_configured("AMP_URL", &endpoint) || {
        if amp_config.exists() {
            std::fs::read_to_string(&amp_config)
                .map(|c| c.contains(&endpoint) || c.contains("localhost:8320"))
                .unwrap_or(false)
        } else {
            false
        }
    ...
    let opencode_configured = if opencode_global_config.exists() {
        // Check if our CodexManager provider is configured
        std::fs::read_to_string(&opencode_global_config)
            .map(|content| content.contains("CodexManager") && content.contains(&endpoint))
            .unwrap_or(false)
    } else {
        false
    };
...
// Configure a CLI agent with CodexManager
#[tauri::command]
    ...
    match agent_id.as_str() {
        ...
        "codex" => {
            ...
            let config_content = format!(
                r#"# CodexManager - Codex Configuration
...
"#,
            ...
            // Write auth.json
            let auth_content = r#"{
  "OPENAI_API_KEY": "CodexManager-local"
}"#;
...
fn configure_claude_code_agent(
    ...
    let env_config = serde_json::json!({
        ...
        "ANTHROPIC_AUTH_TOKEN": "CodexManager-local",
            ...
            if let Ok(mut existing_json) = serde_json::from_str::<serde_json::Value>(&existing) {
                ...
                if let Some(env) = existing_json.get_mut("env") {
                    if let Some(obj) = env.as_object_mut() {
                        // Update CodexManager-related env vars
                        obj.insert(
    ...
    let reference_path = config_dir.join("CodexManager-models.md");
    let reference_content = format!(
        r#"# CodexManager Model Reference for Claude Code
...
> Import your service account JSON in CodexManager to use these models.
...
---
Generated by CodexManager. Run `claude` to start using Claude Code.
"#,
    ...
    Ok(serde_json::json!({
        ...
        "instructions": format!("CodexManager configured for Claude Code. See {} for all available model options from different providers.", reference_path.to_string_lossy())
    }))
}
...
fn configure_factory_droid_agent(
    ...
    let final_config = if config_path.exists() {
        if let Ok(existing) = std::fs::read_to_string(&config_path) {
            if let Ok(mut existing_json) = serde_json::from_str::<serde_json::Value>(&existing) {
                // Get existing custom_models, filter out CodexManager entries, then add new ones
                let mut merged_models: Vec<serde_json::Value> = Vec::new();

                // Keep existing models that are NOT from CodexManager (don't have CodexManager-local api_key)
                if let Some(existing_models) =
                    existing_json.get("custom_models").and_then(|v| v.as_array())
                {
                    for model in existing_models {
                        let is_codex_manager = model
                            .get("api_key")
                            .and_then(|v| v.as_str())
                            .map(|s| s == "CodexManager-local")
                            .unwrap_or(false);
                        if !is_codex_manager {
                ...
                // Add all CodexManager models
                merged_models.extend(codex_manager_models);
                ...
                serde_json::json!({ "custom_models": codex_manager_models })
            }
        } else {
...
fn configure_amp_cli_agent(
    ...
    // The mappings in CodexManager settings are written to CLIProxyAPI config when proxy starts.
    // See: https://help.router-for.me/agent-client/amp-cli.html#model-fallback-behavior

    // CodexManager settings to add/update (only valid Amp CLI settings)
    let codex_manager_settings = serde_json::json!({
    ...
    let final_config = if config_path.exists() {
        if let Ok(existing) = std::fs::read_to_string(&config_path) {
            if let Ok(mut existing_json) = serde_json::from_str::<serde_json::Value>(&existing) {
                // Merge CodexManager settings into existing config
                if let Some(existing_obj) = existing_json.as_object_mut() {
                    if let Some(new_obj) = codex_manager_settings.as_object() {
    ...
    let shell_config = format!(
        r#"# CodexManager - Amp CLI Configuration (alternative to settings.json)
export AMP_URL="{}"
export AMP_API_KEY="CodexManager-local"

# For Amp cloud features, get your API key from https://ampcode.com/settings
# and add it to CodexManager Settings > Amp CLI Integration > Amp API Key
"#,
        amp_endpoint
    );
    ...
    Ok(serde_json::json!({
        ...
        "configPath": config_path.to_string_lossy(),
        "shellConfig": shell_config,
        "instructions": "Amp CLI has been configured. Run 'amp' to start using it. The API key 'CodexManager-local' is pre-configured for local proxy access."
    }))
}
...
fn configure_opencode_agent(
    ...
    // Create or update opencode.json with CodexManager provider
            ...
            if let Ok(mut existing_json) = serde_json::from_str::<serde_json::Value>(&existing) {
                ...
                if let Some(providers) = existing_json.get_mut("provider") {
                    if let Some(obj) = providers.as_object_mut() {
                        obj.insert(
                            "CodexManager".to_string(),
                            opencode_config["provider"]["CodexManager"].clone(),
                        );
                    }
                } else {
    ...
    Ok(serde_json::json!({
        ...
        "configPath": config_path.to_string_lossy(),
        "modelsConfigured": models.len(),
        "instructions": "CodexManager provider added to OpenCode. Run 'opencode' and use /models to select a model (e.g., CodexManager/gemini-2.5-pro). OpenCode uses AI SDK (ai-sdk.dev) and models.dev registry."
    }))
}
...
pub fn append_to_shell_profile(content: String) -> Result<String, String> {
    ...
    // Check if CodexManager config already exists
    if existing.contains("# CodexManager") {
        return Err(
            "CodexManager configuration already exists in shell profile. Please remove it first or update manually."
                .to_string(),
        );
    }
...
// Configure Continue extension with CodexManager endpoint
#[tauri::command]
pub fn configure_continue(state: State<AppState>) -> Result<String, String> {
    ...
    // If config exists and already has CodexManager, update it
    if existing_content.contains("CodexManager") || existing_content.contains(&endpoint) {
        return Ok("Continue is already configured with CodexManager".to_string());
    }

    // Create new config or append to existing
    let new_config = if existing_content.is_empty() {
        format!(
            r#"# Continue configuration - Auto-configured by CodexManager
name: CodexManager Config
version: 0.0.1
schema: v1

models:
  - name: CodexManager (Auto-routed)
    provider: openai
    model: gpt-4
    apiKey: CodexManager-local
    apiBase: {}
...
"#,
        ...
        // Append CodexManager model to existing config
        format!(
            r#"{}
  # Added by CodexManager
  - name: CodexManager (Auto-routed)
    provider: openai
    model: gpt-4
    apiKey: CodexManager-local
    ...
    apiBase: {}"#, endpoint),
        ...
        "cline" => serde_json::json!({
            ...
            "steps": [
                ...
                {
                    "title": "Set Base URL",
                    "description": "Enter the CodexManager endpoint:",
                    "copyable": endpoint.clone()
                },
                {
                    "title": "Set API Key",
                    "description": "Enter: CodexManager-local",
                    "copyable": "CodexManager-local".to_string()
                },
                {
                    "title": "Select Model",
