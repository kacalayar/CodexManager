...
fn build_proxy_config_yaml(config: &AppConfig, config_dir: &std::path::Path) -> Result<String, String> {
    ...
    let mut proxy_config = format!(
        r#"# CodexManager generated config
port: {}
...
fn build_openai_compat_section(config: &AppConfig) -> String {
    ...
    // Add entry for each enabled copilot instance
    for instance in config.copilot.instances.iter() {
        if instance.enabled {
            entries.push(build_copilot_openai_entry_for_instance(instance));
        }
    }
...
fn build_copilot_openai_entry_for_instance(instance: &crate::types::copilot::CopilotInstanceConfig) -> String {
    let port = instance.port;
    let name_suffix = if instance.name.is_empty() { String::new() } else { format!("-{}", instance.name.to_lowercase().replace(" ", "-")) };
    let mut entry = format!("  # GitHub Copilot GPT/OpenAI models (via copilot-api) - {}\n", instance.name);
...
pub async fn start_proxy(
    ...
    // Kill any existing tracked proxy process first
    {
        ...
        if let Some(child) = process.take() {
            println!("[CodexManager] Killing tracked proxy process");
    ...
    {
        // Kill by port
        println!("[CodexManager] Killing any process on port {}", port);
    ...
    for attempt in 0..25 {
            ...
            .await
        {
            ...
            _ => {
                if attempt == 24 {
                    eprintln!("[CodexManager Debug] Proxy not ready after 5s, proceeding anyway");
                }
            }
        }
    }
